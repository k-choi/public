...
한 세션에서 다섯 번 푸시했는데 그 커밋 어느 것에 대해서도 실행이 보이지 않은
...
그때 "이 환경의 푸시는 워크플로를 트리거하지 않는다"고 읽었는데, 틀렸습니다.
그 뒤의 푸시는 실행을 만들었고, 다만 목록에 늦게 나타나고 큐에서 수십 분을
머물렀습니다. 원인은 트리거가 아니라 큐 지연입니다. **진단이 틀려도 확인
방법은 같다**는 것이 요점입니다.
